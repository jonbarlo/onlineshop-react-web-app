...
interface ProductVariantManagerProps {
  ...
  productName?: string; // Add product name for SKU generation
  className?: string;
}
...
export const ProductVariantManager: React.FC<ProductVariantManagerProps> = ({
  ...
  const generateSKU = (color: string, size: string) => {
    ...
    const sizeSlug = size.toUpperCase();
    
    return `${productNameSlug}-${colorSlug}-${sizeSlug}`;
  };
        ...
        <div className="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg border">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <Input
                placeholder="Auto-generated from product name, color, and size"
                value={newVariant.sku}
                onChange={(value) => setNewVariant(prev => ({ ...prev, sku: value }))}
              />
              <p className="mt-1 text-xs text-gray-500">Format: {productName ? generateSKU('#000000', 'M') : 'PRODUCT-NAME-COLOR-SIZE'}</p>
            </div>
          </div>
