...
export const ProductVariantSelector: React.FC<ProductVariantSelectorProps> = ({
  ...
  // Find available sizes for selected color (only in-stock variants)
  ...
  // Find available colors for selected size (only in-stock variants)
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {availableColors.map((color) => {
              ...
              const variant = variants.find(v => v.color === color && v.isActive);
              ...
              return (
                ...
                >
                  {color.startsWith('#') && (
                    <div 
                      className={`w-4 h-4 rounded-full border border-gray-300 shadow-sm ${isOutOfStock ? 'opacity-50' : ''}`}
                      style={{ backgroundColor: color }}
                      title={color}
                    />
                  )}
                  {!color.startsWith('#') && (
                    <span className={`text-sm font-medium ${isOutOfStock ? 'text-gray-500' : 'text-gray-900 dark:text-white'}`}>
                      {color}
                    </span>
                  )}
                  ...
                  {isOutOfStock && (
                    <span className="text-xs text-red-500 font-medium ml-1">{t('admin.sold_out')}</span>
                  )}
                </button>
              );
            })}
            ...
            {availableSizes.map((size) => {
              ...
              return (
                ...
                >
                  ...
                  {isOutOfStock && (
                    <span className="text-xs text-red-500 font-medium absolute -bottom-1 left-1/2 transform -translate-x-1/2 whitespace-nowrap">
                      {t('admin.sold_out')}
                    </span>
                  )}
                </button>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-green-600 dark:text-green-400">
                {t('admin.sku')}: {currentVariant.sku}
              </p>
            </div>
            <div className="text-right">
              <span className="text-sm font-semibold text-green-800 dark:text-green-200">
                {currentVariant.quantity} {t('admin.in_stock')}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-3">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm text-yellow-800 dark:text-yellow-200">
              {t('admin.select_both_color_size')}
            </span>
          </div>
        </div>
      )}
